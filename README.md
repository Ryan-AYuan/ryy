...
- **趣味鎖屏 (Fun Lock Screen)**：
    ...
    - 增加窗口抖動特效，提供生動的視覺反饋。
- **現代化設置彈窗 (Modern Config Modal)**：
    - 採用 **Backdrop Filter (背景模糊)** 增加視覺質感，專注於內容。
    - 優化了排版、字體顏色與輸入框間距，解決了文字重疊與顯示不清的問題。
    - **Rate Limit 監控**：狀態指示燈 (綠/橙/紅) 直觀展示 API 剩餘額度。
    - **智能提示**：根據倉庫類型 (Public/Private) 動態提示 Token 是否必填。
...
## 💻 單文件運行與雲端聯動 (Single File Run & Cloud Integration)

本項目經過優化，支持極致的便攜性與雲端聯動：

### 1. `index.html` 單文件運行
即使您只擁有 `index.html` 這一個文件（沒有下載 `app.js`, `gallery.js` 等其他資源），您依然可以正常打開並運行整個網站。
- **原理**：`index.html` 內部的腳本引用已指向 **GitHub CDN (jsDelivr)**。
- **效果**：它會自動從雲端加載最新的 `app.js`, `gallery.js`, `animation.js` 等依賴文件，確保功能完整。

### 2. 雲端導航與 `upload.html`
- **無縫跳轉**：`index.html` 與 `upload.html` 之間的跳轉鏈接已配置為 **GitHub Pages 在線地址**。
- **獨立性**：`upload.html` 保持了獨立運行的能力，同時也能通過首頁的 "Upload Files" 鏈接直接訪問（前提是已部署到 GitHub Pages）。
- **優勢**：無論您是在本地打開單個文件，還是在線上訪問，都能保證頁面間的導航暢通無阻。

---

## 🛠️ 配置說明 (Configuration)

### 1. GitHub Token 使用規則 (Token Usage Guide)

GitHub Token 並非總是必須的，這取決於您的使用場景 (瀏覽 vs 上傳) 以及倉庫類型 (Public vs Private)。

| 場景 | 公開倉庫 (Public Repo) | 私有倉庫 (Private Repo) | Token 權限要求 |
| :--- | :--- | :--- | :--- |
| **瀏覽/查看 (View)** | **可選 (Optional)**<br>不填 Token 也可以訪問，但 API 限制為 60次/小時。<br>填寫後提升至 5000次/小時。 | **必須 (Required)**<br>必須填寫 Token 才能獲取文件列表和內容。 | `Contents: Read-only` |
| **上傳文件 (Upload)** | **必須 (Required)**<br>GitHub API 不允許匿名上傳。 | **必須 (Required)** | `Contents: Read and write` |

> **💡 關於 Fine-grained Tokens**：
> 即使您的倉庫是公開的，您也可以使用為私有倉庫創建的 Fine-grained Token，只要該 Token 的權限範圍 (Repository access) 包含當前倉庫即可。

### 2. ❓ 常見問題：數據來源 (Data Source FAQ)

**Q: 當我在另一台電腦 (本地) 點擊 `index.html`，我訪問的是本地數據還是 GitHub 數據？**

**A: 您訪問的是 GitHub 雲端數據。**

即使您是在本地打開文件 (`file:///.../index.html`)，網頁的運作邏輯如下：
1.  **獲取列表**：網頁會調用 GitHub API 獲取倉庫中最新的文件列表。
2.  **加載內容**：
    *   **項目 (Projects)**：鏈接指向 GitHub Pages (在線預覽)。
    *   **資源 (Photo/Music)**：鏈接指向 Raw GitHub Content (雲端文件)。

**這意味著：**
*   您無需手動將圖片複製到每一台電腦。
*   只要有一台電腦上傳了新文件到 GitHub，其他所有設備 (包括本地打開的 `index.html`) 刷新後都能看到最新內容。
*   **前提**：電腦必須聯網，且倉庫必須開啟 GitHub Pages。

### 3. 如何開啟 GitHub Pages (在線訪問)
為了讓網站可以隨時隨地訪問（生成在線鏈接），您需要開啟 GitHub Pages 功能。對於**公開倉庫 (Public Repository)**，此功能是**完全免費**的。

**設置步驟：**
1. 進入 GitHub 倉庫頁面，點擊頂部的 **"Settings"**。
2. 在左側菜單找到 **"Code and automation"** -> **"Pages"**。
3. 在 **"Source"** 下選擇 **"Deploy from a branch"**。
4. 在 **"Branch"** 選擇 **`main`** (或 master)，文件夾選 **`/ (root)`**，點擊 **"Save"**。
5. 等待幾分鐘後，刷新頁面，您會看到綠色的在線訪問鏈接 (例如 `https://Ryan-AYuan.github.io/ryy/`)。

### 4. 如何獲取 GitHub Personal Access Token (PAT)
